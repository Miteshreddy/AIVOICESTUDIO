...
export function VoiceSelector({ service }: { service: ServiceType }) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute left-0 right-0 z-50 mt-1.5 max-h-72 overflow-auto rounded-lg border border-[#23252a] bg-[#17181d] shadow-xl p-1 flex flex-col gap-0.5">
          ...
          {voices.map((voice) => (
            <div
              ...
              className={`flex items-center justify-between px-2.5 py-2 rounded-md hover:cursor-pointer transition-colors ${
            ...
            >
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-xs truncate">{voice.name}</span>
              </div>
              {voice.isCloned && (
